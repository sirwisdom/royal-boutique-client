...
const ItemInfo = (props) => {
  ...
  useEffect(() => {
    axios
      .get(`${url}/product/getASingleProduct/${params.itemId}`)
      .then((res) => {
        let tempImages = [];
      ...
      <section className="iteminfo-section">
        ...
        <div className="iteminfo-text-div">
          <div className="iteminfo-text-div-inner">
            ...
            <br />
            {itemInfo.brand ? (
              <h4 className="iteminfo-size">
                <span className="size-inner">Brand:</span> {itemInfo.brand}
              </h4>
            ) : null}
            {itemInfo.bust ? (
              <h4 className="iteminfo-size">
                <span className="size-inner">Bust/Chest:</span>{" "}
                <span className="size-measurement">
                  {itemInfo.bust}in or {inchesToCentimeter(itemInfo.bust)}cm
                </span>
              </h4>
            ) : null}
            {itemInfo.sleeveLength ? (
              <h4 className="iteminfo-size">
                <span className="size-inner">Length of Sleeve:</span>{" "}
                <span className="size-measurement">
                  {itemInfo.sleeveLength}in or{" "}
                  {inchesToCentimeter(itemInfo.sleeveLength)}cm
                </span>
              </h4>
            ) : null}
            {itemInfo.shoulder ? (
              <h4 className="iteminfo-size">
                <span className="size-inner">Length of Shoulder:</span>{" "}
                <span className="size-measurement">
                  {itemInfo.shoulder}in or{" "}
                  {inchesToCentimeter(itemInfo.shoulder)}cm
                </span>
              </h4>
            ) : null}
            {itemInfo.waist ? (
              <h4 className="iteminfo-size">
                <span className="size-inner">Length of Waist:</span>{" "}
                <span className="size-measurement">
            ...
            {itemInfo.hipSize ? (
              <h4 className="iteminfo-size">
                <span className="size-inner">Length of Hip:</span>{" "}
                <span className="size-measurement">
                  {itemInfo.hipSize}in or {inchesToCentimeter(itemInfo.hipSize)}
                  cm
                </span>
              </h4>
            ) : null}
            ...
            {itemInfo.length ? (
              <h4 className="iteminfo-size">
                <span className="size-inner">Length:</span>{" "}
                <span className="size-measurement">
            ...
            <br />
            <button
              className="iteminfo-add-to-cart-btn"
